---
name: systems-engineering-saas
description: >-
  one-piece ハードウェア向けシステムエンジニアリング SaaS の要求階層、AIV 計画、サブシステム設計パッケージ、
  検証（分析／試験／検査）、検証プラットフォームニーズ、コンプライアンスマトリクスをモデル化する。
  要求の展開、トレーサビリティ、V&V 計画、プラットフォーム仕様、packages/domain 型の拡張時に使う。
---

# システムエンジニアリング SaaS（one-piece）

## スコープ

**要求 → 設計 → 検証 → 証拠**に触れるドメイン型、API、UX、エージェントプロンプトの作業に適用する。

## 要求レベル（上から下）

1. **ミッション要求** — プログラムの意図
2. **システム要求** — システム全体の振る舞いと制約；**AIV（Assembly and Integration Verification）** 計画を含む
3. **運用要求** — 使用、展開、ロジスティクス、保守
4. **サブシステム要求** — サブシステムごとの配分と制約

**システム要求**は**運用要求**と**サブシステム要求**の**両方**を駆動する。暗黙の順序だけでなく、明示的なトレースリンクで分岐を表現する。

## 安定した意図 vs 派生トレード

**ユーザ／ミッション／顧客向け**意図は端から端まで検証する。**派生**下位要求と仕様は、試験や分析がより良い配分を示せば変更してよい—常に**トレース可能な根拠**（決定、分析 ID、試験 ID、または免除）を付ける。[AGENT.md](../../AGENT.md) のエンジニアリング哲学を参照。

## 試験の厳しさと統合

- **試験可能**な設計を優先；統合実行（例: HITL 様）は検証活動 + **検証プラットフォーム**アーティファクトにマップする。
- 有用なら試験に**目的**を付与: development（マージンを学ぶ）→ qualification（有界最悪ケース）→ acceptance（仕上げ）— ライフサイクルに沿って形式性が上がる。

## サブシステム要求の出力

サブシステム要求は次に接続する:

- **サブシステム設計**（設計パッケージ + **分析結果**）
- **サブシステム検証活動計画**（分析・試験・検査—各 **レポート** 付き）
- **検証プラットフォーム要求**と**仕様**

## 整合性

サブシステム設計は相互接続されている。実質的な編集の後:

- **横方向**整合性（インターフェース、環境、共有仮定）
- システム・ミッション要求への**上方**整合

## コンプライアンスマトリクス

UX 目標: **Excel 並みにシンプル**。行 = 要求（または検証目的）；列 = 証拠；セル = ステータス + アーティファクトリビジョンへのリンク。正規化ストレージを優先し、マトリクスは**ビュー／エクスポート**とする。

## 真実の源泉

- オーケストレーション: [AGENT.md](../../AGENT.md)
- ライブアーキテクチャ: [docs/INFORMATION_ARCHITECTURE.md](../../docs/INFORMATION_ARCHITECTURE.md)

有意な変更の後は [docs/DEVELOPMENT_PROGRESS.md](../../docs/DEVELOPMENT_PROGRESS.md) に追記する。
