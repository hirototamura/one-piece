...
**ミッションから検証まで**、ハードウェア製品の要件・設計・インターフェース・検証を**一つのトレーサブルなグラフ**で管理する SaaS の概念実証（PoC）です。**あらゆる規模のチーム**を対象としつつ、ロードマップでは**小規模チームを最優先**に、スプレッドシート並みに扱えるコンプライアンスビューを目指します。
...
要件・ICD・設計パラメータ・制約・CAD・検証活動・トレースエッジを**一つのグラフ**として探索します。マトリクスやツリーはこのグラフの**投影**であり、正本はここに集約されます。

### 2. Requirements — 階層要件ツリー

`mission` → `system` → `operational` → `subsystem` の階層で要件を表示し、選択ノードの詳細・ライフサイクル（`draft` / `under_review` / `baseline`）・上流トレースを確認できます。システム要件には **AIV**（Assembly and Integration Verification）計画を紐づけます。
...
`DesignParameter`（値・単位・境界・学科タグ）と `DesignConstraint` を一覧します。`actsAsFunctionalRequirement` が立った制約は、機能要件と同様に**配分と V&V** に参加します。
...
### 9. Compliance matrix — 要件 ↔ 証拠

行＝要件（または検証対象）、列＝証拠（テストケース・解析 ID・検査記録）、セル＝ステータス（planned / passed / failed / waived）。Excel 並みの読みやすさを UX 目標とし、下層は正規化トレースから生成します。
...
ハードウェアプログラムでは、要件の曖昧さ・サブシステム間のインターフェース齟齬・検証証拠の散逸が、統合試験の直前まで表面化しません。現実のチームは次のループを回します。
...
| 間抜けな要件を減らす    | 曖昧さを減らし、検証可能な記述へ            |
| 学際設計          | 機械・電気・熱・ソフト・運用・試験を一グラフで     |
| 早期試験・自動化      | 開発試験を早く回し、人間判断はベースラインと異常に集中 |


詳細は [docs/PROJECT_PLAN.md](docs/PROJECT_PLAN.md) と [AGENT.md](AGENT.md) を参照してください。
...
| **Compliance matrix**     | 要件 ↔ 証拠のグリッド（単純ステータス + アーティファクト参照）                        |
...
- **派生仕様・下位要件** — 設計中にトレード可能。ただし**根拠・決定・免除へのリンク必須**
...
| アクター                 | 役割                    | 例                                 |
| -------------------- | --------------------- | --------------------------------- |
| **Human engineer**   | ベースライン・重大判断・現実世界との接点  | 要件編集、免除承認                         |
| **Logic automation** | 再現可能・決定論的             | `one-piece-sync`、試験ランナー、SSOS 取り込み |
| **AI agent**         | ポリシー内ドラフト（デフォルト ~20%） | 要件文案、パラメータ提案、co-design 反復         |
...
| 領域                          | 状態                | 参照                                                                             |
| --------------------------- | ----------------- | ------------------------------------------------------------------------------ |
| ドメインカーネル（`packages/domain`） | **利用可能**          | [docs/INFORMATION_ARCHITECTURE.md](docs/INFORMATION_ARCHITECTURE.md)           |
...
| 本番 API・永続 DB・マルチテナント        | **未実装**           | [docs/PROJECT_PLAN.md](docs/PROJECT_PLAN.md)                                   |
...
| ドキュメント                                                               | 対象読者            | 内容                                 |
| -------------------------------------------------------------------- | --------------- | ---------------------------------- |
| [AGENT.md](./AGENT.md)                                               | エージェント・コントリビュータ | 役割分担、ワークフロー、人間ゲート、ドキュメント義務         |
| [docs/PROJECT_PLAN.md](docs/PROJECT_PLAN.md)                         | プロダクト・開発        | フェーズ、バックログ、リスク、成功指標                |
| [docs/INFORMATION_ARCHITECTURE.md](docs/INFORMATION_ARCHITECTURE.md) | モデラー・設計者        | SSOT、アーティファクト、トレース、マトリクス、co-design |
| [docs/DEVELOPMENT_PROGRESS.md](docs/DEVELOPMENT_PROGRESS.md)         | 全員              | 時系列の変更ログ                           |
| [docs/README.md](docs/README.md)                                     | 全員              | 上記ドキュメントの索引                        |
...
| スキル                                        | 用途                     |
| ------------------------------------------ | ---------------------- |
| `.cursor/skills/systems-engineering-saas/` | 要件階層、V&V、マトリクス、ドメイン型拡張 |
...
| パス                             | 用途                                    |
| ------------------------------ | ------------------------------------- |
| `packages/domain/`             | 共有ドメインモデル（要件、グラフ、トレース、co-design、ポリシー） |
...
| P2 Web UI    | 要件ツリー・マトリクス・レビュー（PoC 済み、永続化はこれから） |
