...
This file is the **control tower** for AI agents working on one-piece: an **AI Systems Engineering as a Service (SEaaS)** for **teams of any size**—requirements, design, verification, and traceability for hardware. **Domain authority** between humans and agents is adjustable: when humans hold 100% authority, agents draft, cross-check, and keep artifacts consistent; as AI agents learn progressively, they **transfer human expertise** into the agent stack; when agents hold 100% authority, they run the full workflow without human intervention to iterate hypotheses at high speed.

## North star

Enable **every scale** of builder—individuals, startups, labs, and large programs—to run **autonomous systems engineering powered by AI** in hardware development (mission through verification) using structured artifacts, traceability, and compliance views that can stay as approachable as an Excel matrix. Use **Reinforcement Learning from Human Feedback (RLHF)** so explainability is backed by documentation. **How they meet the platform differs by team size** (self-serve vs. governance-heavy programs); the product should grow **touchpoints** for larger orgs without abandoning clarity for small ones.
...
| Living product docs       | `docs/ja/`, `docs/en/` | Plan, information architecture, progress log—**update as development advances** (per language) |
...
| Role                         | Responsibility                                                                                                                                                                                                                   |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Orchestrator**             | Breaks work into slices, enforces traceability rules, updates `docs/en/DEVELOPMENT_PROGRESS.md`, resolves conflicts between agents, and may run bounded `CoDesignRun` loops in AI-100 mode for derived/standard-tier exploration |
...
- Append a short entry to `docs/en/DEVELOPMENT_PROGRESS.md` (date, summary, links to PRs/commits if applicable)  
- If the information model or workflows shift, update `docs/en/INFORMATION_ARCHITECTURE.md` or `docs/en/PROJECT_PLAN.md` in the same change set when practical
...
*Version: 0.2 — see `docs/en/PROJECT_PLAN.md` (external influences) and `docs/en/INFORMATION_ARCHITECTURE.md`.*